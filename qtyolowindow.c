    ...
    : QMainWindow(parent)
{
	...
	cv::Mat img0 = cv::imread("C:/Users/tianmingYun/Desktop/cvimage.jpg");
	...
	//connect(img_thread, SIGNAL(detectSignal(const QImage&)), this, SLOT(refreshimage(const QImage&)));
...
{	
	if (IsCamOpen_flag == 1)//如果相机打开了才能采集
	{
		...
		//创建一个线程用于检测与刷新
		//img_thread = new ImageThread();
		//connect(img_thread, SIGNAL(detectSignal(const cv::Mat&)), this, SLOT(refreshimage(const cv::Mat&)));
		img_thread->start();
	}
	else
		QMessageBox::critical(nullptr, "Error", "Camera is not opened!");
